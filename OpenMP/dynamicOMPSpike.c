 ...
 * g++ -fopenmp -O3 -Wall -pedantic dynamicOMPSpike.c -o dspike
 *
 * This program dynamically allocates threads at each level of recursion.
 *
 ...
 * ./dspike ##, ## = size of matrix. must be a size 2^n.
 *
 */
...
void rSpike(double* A,double *B, double *V, double *W, int type, int size, int depth){
	...
	if(partition_size ==2){
		...
		if(type==0){
	    		#pragma omp parallel sections num_threads(2) if(depth>0)
			{
			#pragma omp section
